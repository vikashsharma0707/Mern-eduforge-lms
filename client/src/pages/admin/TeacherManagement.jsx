...
//   const refresh = () => userApi.list({ role: 'teacher' }).then(r => setUsers(r.data));
//   useEffect(refresh, []);
...
export default function TeacherManagement() {
  ...
  const refresh = async () => {
    try {
      setLoading(true);
      const res = await userApi.list({ role: 'teacher' });
      setUsers(res.data || []);
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="container page">
      ...
      {users.length === 0 && !loading && <p className="muted">No teachers found.</p>}
    </div>
  );
}