...
import { toast } from 'react-toastify';
import {
  Upload, BookOpen, DollarSign, Type, FileText,
  Eye, EyeOff, ChevronRight, AlertCircle, Tag
} from 'lucide-react';
...
export default function CreateCourse() {
  ...
  useEffect(() => {
    ...
    categoryApi
      ...
      .catch(() => {
        setCats([]);
        toast.error('Could not load categories — check /api/categories route and seed the DB');
      })
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      Object.entries(form).forEach(([k, v]) => {
        // Don't send empty category — backend will store null
      ...
      const r = await courseApi.create(fd);
      toast.success('🎉 Course created successfully!');
      navigate(`/teacher/courses/${r.data.course?._id || r.data._id}/edit`);
    } catch (err) {
      toast.error(err.response?.data?.message || 'Failed to create course');
    } finally {
      setLoading(false);
  ...
  const containerVariants = {
    hidden: { opacity: 0, y: 20 },
    visible: { opacity: 1, y: 0, transition: { duration: 0.6, staggerChildren: 0.08, delayChildren: 0.1 } },
  };
  const itemVariants = {
    hidden: { opacity: 0, y: 15 },
    visible: { opacity: 1, y: 0, transition: { duration: 0.4 } },
        ...
        >
          ...
          <motion.div variants={itemVariants} className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-semibold text-white mb-3">
                <div className="flex items-center gap-2">
                  <BookOpen className="w-4 h-4 text-green-400" /> Level
                </div>
              </label>
              <select
                value={form.level}
          ...
          <motion.div variants={itemVariants} className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-semibold text-white mb-3">
                <div className="flex items-center gap-2">
                  <Tag className="w-4 h-4 text-pink-400" /> Category
              ...
              <select
                value={form.category}
                onChange={(e) => setForm({ ...form, category: e.target.value })}
                className="w-full bg-gradient-to-r from-white/5 to-white/[0.02] border border-white/10 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-pink-500/50 focus:ring-2 focus:ring-pink-500/20 transition-all cursor-pointer"
              >
                {catsLoading ? (
                  <option value="">Loading categories...</option>
                ) : cats.length === 0 ? (
                  <option value="">No categories — run seed script</option>
                ) : (
                  <>
                    <option value="">Select a category</option>
                    {cats.map((c) => (
              ...
              {!catsLoading && cats.length === 0 && (
                <p className="mt-2 text-xs text-amber-400 flex items-center gap-1">
                  ...
                  Run: <code className="bg-white/10 px-1 rounded">node utils/seedCategories.js</code>
                </p>
              )}
            </div>
            <div>
          ...
          <motion.div variants={itemVariants}>
            <label className="block text-sm font-semibold text-white mb-3">
              <div className="flex items-center gap-2">
                <Upload className="w-4 h-4 text-pink-400" /> Thumbnail Image
              </div>
            </label>
            <div className="flex gap-4">
              <div className="flex-1">
            ...
            <div className="bg-gradient-to-r from-white/5 to-white/[0.02] border border-white/10 rounded-lg p-4">
              <label className="flex items-center gap-3 cursor-pointer">
                ...
                <div>
                  <p className="text-sm font-medium text-white flex items-center gap-2">
                    {form.isPublished ? (
                      <><Eye className="w-4 h-4 text-green-400" /> Publish immediately</>
                    ) : (
                      <><EyeOff className="w-4 h-4 text-slate-400" /> Save as draft</>
                    )}
                  </p>
                  <p className="text-xs text-slate-400 mt-1">
                    {form.isPublished
